...
(function ($) {
	...
	WCP.ColorPicker = function ( elm, options ) {
		...
		// 2. input.value
		// 3. default
		this.color = { h: 0, s: 0, v: 1, r: 1, g: 1, b: 1, a: 1 , l: 1 };
	...
	WCP.ColorPicker.createWidget = function() {
		...
		var $widget = $(
			"<div class='jQWCP-wWidget'>" + 
				...
				"</div>" +
				"<div class='jQWCP-wBrightness jQWCP-slider-wrapper'>" +
					"<canvas class='jQWCP-wBrightnessSlider jQWCP-slider jQWCP-wPreviewBox' width='1' height='1' title='Brightness'></canvas>" +
					"<span class='jQWCP-wBrightnessCursor jQWCP-scursor'></span>" +
				"</div>" +
	...
	WCP.ColorPicker.prototype.refreshWidget = function() {
		...
		for(var i in options.sliders) {
			...
			switch(this.options.sliders[i]) {
				...
				case 'l':
					$slider = $widget.find('.jQWCP-wBrightness'); break;
				case 'p':
		...
		$widget.find('.jQWCP-slider:not(.jQWCP-wPreviewBox)').attr('height', sliderHeight);
	...
	WCP.ColorPicker.prototype.redrawSliders = function( force ) {
		...
		/// BRIGHTNESS ///
		// Duplicate of preview box
		var $brightnessSlider = $widget.find('.jQWCP-wBrightnessSlider');
		if(!$brightnessSlider.hasClass('hidden') || force) {
			var brightnessSliderCtx = $brightnessSlider.get(0).getContext('2d');
			brightnessSliderCtx.fillStyle = "rgba(" + R + "," + G + "," + B + "," + A + ")";
			brightnessSliderCtx.clearRect(0, 0, 1, 1);
			brightnessSliderCtx.fillRect(0, 0, 1, 1);
		}
	...
	WCP.ColorPicker.prototype.updateSliders = function() {
		...
		// Brightness
		var $brightnessSlider = $widget.find('.jQWCP-wBrightnessSlider');
		if(!$brightnessSlider.hasClass('hidden')) {
			var $brightnessCursor = $widget.find('.jQWCP-wBrightnessCursor');
			$brightnessCursor.css('top', ((1 - color.l) * $brightnessSlider.height()) + 'px');
		}
	...
	WCP.ColorPicker.prototype.updateActiveControl = function( e ) {
		...
		/// SLIDER CONTROL ///
			...
			// Brightness
			if($control.hasClass('jQWCP-wBrightness')) {
	...
	WCP.ColorPicker.prototype.setColor = function( color ) {
		...
		else if(color.l != null && color.r != null) {
			return this.setRgbal(color.r, color.g, color.b, color.a, color.l);
		}
		else if(color.l != null && color.h != null) {
			return this.setHsval(color.h, color.s, color.v, color.a, color.l);
		}
		...
		else if(color.l != null) {
			return this.setBrightness(color.l);
		}
	 ...
	 * Set color using RGBA combination.
	 */
	WCP.ColorPicker.prototype.setRgbal = function( r, g, b, a, l ) {
		var color = this.color;
		color.r = r;
		color.g = g;
		color.b = b;
	 ...
	 * Set color using HSVA combination.
	 */
	WCP.ColorPicker.prototype.setHsval = function( h, s, v, a, l ) {
		var color = this.color;
		color.h = h;
		color.s = s;
		color.v = v;
	 ...
	 * Set alpha value.
	 */
	WCP.ColorPicker.prototype.setBrightness = function( value ) {
		this.color.l = value;
